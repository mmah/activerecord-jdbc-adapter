...
module ::ArJdbc
  module PostgreSQL
    ...
    def pg_insert(sql, name = nil, pk = nil, id_value = nil, sequence_name = nil, binds = [])
     ...
     # Returns the list of a table's column names, data types, and default values.
      ...
      def query(sql, name = nil) #:nodoc:
          return @connection.execute(sql).collect{|res| res.values}
      end
    ...
    def pg_columns(table_name, name=nil)
      ...
      Benchmark.bm do |x|
        x.report do
          ret = pg_columns_inner(table_name, name)
    #      p "WUT? #{ret.class.name} #{ret.to_s}"
    ...
    def pg_columns_inner3(table_name, name=nil)
      column_definitions(table_name).collect do |name, type, default, notnull|
        ActiveRecord::ConnectionAdapters::PostgreSQLColumn.new(name, type, default, notnull == 'f')
      end
    end
